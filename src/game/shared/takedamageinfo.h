...
public:
	...
	void			SetDamageForceScale(float newdamageforcescale ) { m_flDamageForceScale = newdamageforcescale; }
	float			GetDamageForceScale() const { return m_flDamageForceScale; }
...
protected:
	void			Init( CBaseEntity *pInflictor, CBaseEntity *pAttacker, CBaseEntity *pWeapon, const Vector &damageForce, const Vector &damagePosition, const Vector &reportedPosition, float flDamage, int bitsDamageType, int iKillType, float ScaleDamageForce = 1.0f );
...
{
	m_flDamageForceScale = flScaleAmount;
