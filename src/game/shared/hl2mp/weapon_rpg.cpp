...
#define MISSILE_EXPLOSION_FORCE 500.0f
...
void CMissile::DoExplosion( void )
{
	...
	ExplosionCreate( GetAbsOrigin(), GetAbsAngles(), GetOwnerEntity(), GetDamage(), GetDamage() * 2, 
		SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE, MISSILE_EXPLOSION_FORCE, this);
}
...
void CMissile::MissileTouch( CBaseEntity *pOther )
{
	...
	// Don't hit weapons
	if ( pOther->GetCollisionGroup() == COLLISION_GROUP_WEAPON || pOther->GetCollisionGroup() == COLLISION_GROUP_FLAG )
	{
