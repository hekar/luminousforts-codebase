...
// Why is this so complicated? For button presses...
...
void IN_FreezeDown( const CCommand &args ) { KeyDown( &in_freeze, args[1] ); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_FREEZE, s_ClearInputState, &in_freeze, bResetState );
...
static ConCommand endfreeze( "+freeze", IN_FreezeDown );
static ConCommand startfreeze( "-freeze", IN_FreezeUp );
static ConCommand endDropFlag( "+dropflag", IN_DropFlagDown );
static ConCommand startDropFlag( "-dropflag", IN_DropFlagUp );
