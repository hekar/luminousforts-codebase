...
/*===============================================================
	...
	Valve's old SectionedList from Episode 1
	Will be modifying this to suit needs for modification
...
//-----------------------------------------------------------------------------
class COldSectionHeader : public Label
{
	DECLARE_CLASS_SIMPLE( COldSectionHeader, Label );

public:
	COldSectionHeader(OldSectionedList *parent, const char *name, int sectionID) : Label(parent, name, "")
	{
		m_pListPanel = parent;
	...
	COldSectionHeader(OldSectionedList *parent, const wchar_t *name, int sectionID) : Label(parent, "SectionHeader", "")
	{
		SetText(name);	
		SetVisible(false);
		m_pListPanel = parent;
	...
	void PerformLayout()
	{
		...
		if (colCount != GetImageCount())
		{
			...
			for (int i = 0; i < colCount; i++)
			{
				...
				IImage *image = NULL;
				if (columnFlags & OldSectionedList::HEADER_IMAGE)
				{
		...
		for (int repeat = 0; repeat <= 1; repeat++)
		{
			...
			for (int i = 0; i < colCount; i++)
			{
				...
				if (!(columnFlags & OldSectionedList::COLUMN_RIGHT))
				{
...
private:
	...
	OldSectionedList *m_pListPanel;
};

//-----------------------------------------------------------------------------
// Purpose: Individual items in the list
//-----------------------------------------------------------------------------
class COldItemButton : public Label
{
	DECLARE_CLASS_SIMPLE( COldItemButton, Label );

public:
	COldItemButton(OldSectionedList *parent, int itemID) : Label(parent, NULL, "< item >")
	{
		m_pListPanel = parent;
		m_iID = itemID;
		m_pData = NULL;
	...
	virtual void PerformLayout()
	{
		...
		else
		{
			if (colCount != GetImageCount())
			{
				...
				{
					int columnFlags = m_pListPanel->GetColumnFlagsBySection(m_iSectionID, i);
					if (!(columnFlags & OldSectionedList::COLUMN_IMAGE))
					{
			...
			for (int i = 0; i < colCount; i++)
			{
				...
				if (columnFlags & OldSectionedList::COLUMN_IMAGE)
				{
					...
					if (m_pListPanel->m_pImageList)
					{
						...
						else
						{
							...
							// Assert(!("Image index out of range for ImageList in OldSectionedList"));
						}
					}
					else
					{
						Assert(!("Images columns used in OldSectionedList with no ImageList set"));
					}
				}
				else
				{
					...
					if (textImage)
					{
						// Louti 04/02/07 - new score system
						if (m_pData->FindKey( keyname ))
						{
							if(m_pData->FindKey( keyname )->GetDataType( ) == KeyValues::TYPE_FLOAT)
							{
								char t[32] = {0};
								float flValue = m_pData->GetFloat( keyname, 0.0f );
								if (ceilf( flValue ) != flValue) // we got .5 ?
									sprintf( t, "%3.1f", flValue );
								else
									sprintf( t, "%i", (int)flValue );
								textImage->SetText( t );
							}
							else
						...
						// set the text color based on the selection state - if one of the children of the OldSectionedList has focus, then 'we have focus' if we're selected
						...
						if ( !m_bOverrideColors )
						{
							...
							else if (columnFlags & OldSectionedList::COLUMN_BRIGHT)
							{
				...
				if (i == 0 && !(columnFlags & OldSectionedList::COLUMN_IMAGE))
				{
				...
				{
					if (columnFlags & OldSectionedList::COLUMN_CENTER)
					{
						SetImageBounds(i, xpos + imageWide / 2, xpos + wide - COLUMN_DATA_GAP - imageWide / 2);
					}
					else if (columnFlags & OldSectionedList::COLUMN_RIGHT)
					{
	...
	virtual void PaintBackground()
	{
		...
		{
            VPANEL focus = input()->GetFocus();
            // if one of the children of the OldSectionedList has focus, then 'we have focus' if we're selected
            if (HasFocus() || (focus && ipanel()->HasParent(focus, GetVParent())))
            {
	...
	virtual void OnMouseDoublePressed(MouseCode code)
	{
		if (code == MOUSE_LEFT)
		{
			...
			m_pListPanel->OnKeyCodeTyped((vgui::KeyCode)KEY_ENTER);
		}
		else
		{
			OnMousePressed(code);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
OldSectionedList::OldSectionedList(vgui::Panel *parent, const char *name) : BaseClass(parent, name)
{
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
OldSectionedList::~OldSectionedList()
{
}
...
//-----------------------------------------------------------------------------
void OldSectionedList::ReSortList()
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::PerformLayout()
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::LayoutPanels(int &contentTall)
{
	...
	for (int sectionIndex = 0; sectionIndex < m_Sections.Size(); sectionIndex++)
	{
		...
		else
		{
			...
			{
				COldItemButton *item = m_SortedItems[i]; //items[i];
...
//-----------------------------------------------------------------------------
void OldSectionedList::ScrollToItem(int iItem)
{
	int tall = GetSectionTall();
	int itemX, itemY ;
...
//-----------------------------------------------------------------------------
void OldSectionedList::ApplySchemeSettings(IScheme *pScheme)
{
	BaseClass::ApplySchemeSettings(pScheme);
...
// Purpose: 
//-----------------------------------------------------------------------------
void OldSectionedList::ApplySettings(KeyValues *inResourceData)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetProportional(bool state)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetVerticalScrollbar(bool state)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::AddSection(int sectionID, const char *name, SectionSortFunc_t sortFunc)
{
	COldSectionHeader *header = SETUP_PANEL(new COldSectionHeader(this, name, sectionID));
...
// Purpose: adds a new section
//-----------------------------------------------------------------------------
void OldSectionedList::AddSection(int sectionID, const wchar_t *name, SectionSortFunc_t sortFunc)
{
	COldSectionHeader *header = SETUP_PANEL(new COldSectionHeader(this, name, sectionID));
	AddSectionHelper(sectionID, header, sortFunc);
}
...
//-----------------------------------------------------------------------------
void OldSectionedList::AddSectionHelper(int sectionID, COldSectionHeader *header, SectionSortFunc_t sortFunc)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::RemoveAllSections()
{
...
//-----------------------------------------------------------------------------
bool OldSectionedList::AddColumnToSection(int sectionID, const char *columnName, const char *columnText, int columnFlags, int width)
{
	wchar_t wtext[64];
...
//-----------------------------------------------------------------------------
bool OldSectionedList::AddColumnToSection(int sectionID, const char *columnName, const wchar_t *columnText, int columnFlags, int width)
{
...
//-----------------------------------------------------------------------------
bool OldSectionedList::ModifyColumn(int sectionID, const char *columnName, const wchar_t *columnText)
{
	int index = FindSectionIndexByID(sectionID);
	if (index < 0)
		return false;
...
//-----------------------------------------------------------------------------
int OldSectionedList::AddItem(int sectionID, const KeyValues *data)
{
...
//-----------------------------------------------------------------------------
bool OldSectionedList::ModifyItem(int itemID, int sectionID, const KeyValues *data)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void OldSectionedList::SetItemFgColor( int itemID, Color color )
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetSectionFgColor(int sectionID, Color color)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetSectionAlwaysVisible(int sectionID, bool visible)
{
	if (!m_Sections.IsValidIndex(sectionID))
		return;
...
//-----------------------------------------------------------------------------
bool OldSectionedList::RemoveItem(int itemID)
{
	if ( !m_Items.IsValidIndex(itemID) )
		return false;
...
//-----------------------------------------------------------------------------
int OldSectionedList::GetColumnCountBySection(int sectionID)
{
	int index = FindSectionIndexByID(sectionID);
	if (index < 0)
		return NULL;
...
//-----------------------------------------------------------------------------
const char *OldSectionedList::GetColumnNameBySection(int sectionID, int columnIndex)
{
	int index = FindSectionIndexByID(sectionID);
...
//-----------------------------------------------------------------------------
const wchar_t *OldSectionedList::GetColumnTextBySection(int sectionID, int columnIndex)
{
	int index = FindSectionIndexByID(sectionID);
	if (index < 0 || columnIndex >= m_Sections[index].m_Columns.Size())
		return NULL;
...
//-----------------------------------------------------------------------------
int OldSectionedList::GetColumnFlagsBySection(int sectionID, int columnIndex)
{
	int index = FindSectionIndexByID(sectionID);
	if (index < 0)
		return 0;
...
// Purpose: 
//-----------------------------------------------------------------------------
int OldSectionedList::GetColumnWidthBySection(int sectionID, int columnIndex)
{
	int index = FindSectionIndexByID(sectionID);
	if (index < 0)
		return 0;
...
//-----------------------------------------------------------------------------
int OldSectionedList::FindSectionIndexByID(int sectionID)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::OnSliderMoved()
{
	InvalidateLayout();
	Repaint();
}
...
//-----------------------------------------------------------------------------
void OldSectionedList::OnMouseWheeled(int delta)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::OnSizeChanged(int wide, int tall)
{
...
//-----------------------------------------------------------------------------
// Purpose: Determine selection item
//-----------------------------------------------------------------------------
void OldSectionedList::OnCursorEntered()
{
...
// Purpose: Determine selection item
//-----------------------------------------------------------------------------
void OldSectionedList::OnCursorExited()
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::OnMousePressed(MouseCode code)
{
	for (int i = 0; i < GetItemCount(); i++)
	{
		int x, y, w, h;
		// Column is ignored in this version
		GetCellBounds( i, 0, x, y, w, h );
		int curx, cury;
		input()->GetCursorPosition( curx, cury );
		
		if ( curx > x && cury > y && curx < (x + w) && cury < (y + h) )
		{
			m_iDesiredItem = i;
			break;
		}
	...
	if ( m_iDesiredItem > -1 )
	{
		SetSelectedItem( m_Items[ m_iDesiredItem ] );
	}
}
...
//-----------------------------------------------------------------------------
void OldSectionedList::OnKeyCodeTyped(KeyCode code)
{
	if (m_hEditModePanel.Get())
	{
...
//-----------------------------------------------------------------------------
void OldSectionedList::DeleteAllItems()
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetSelectedItem(COldItemButton *item)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int OldSectionedList::GetSelectedItem()
{
	if (m_hSelectedItem.Get())
	{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetSelectedItem(int itemID)
{
	if ( m_Items.IsValidIndex(itemID) )
	{
		SetSelectedItem(m_Items[itemID]);
...
//-----------------------------------------------------------------------------
KeyValues *OldSectionedList::GetItemData(int itemID)
{
	Assert(m_Items.IsValidIndex(itemID));
	if ( !m_Items.IsValidIndex(itemID) )
		return NULL;
...
//-----------------------------------------------------------------------------
int OldSectionedList::GetItemSection(int itemID)
{
	if ( !m_Items.IsValidIndex(itemID) )
		return -1;
...
//-----------------------------------------------------------------------------
bool OldSectionedList::IsItemIDValid(int itemID)
{
...
// Purpose: returns true if the itemID is valid for use
//-----------------------------------------------------------------------------
int OldSectionedList::GetHighestItemID()
{
...
//-----------------------------------------------------------------------------
int OldSectionedList::GetItemCount()
{
...
// Purpose: item iterators
//-----------------------------------------------------------------------------
int OldSectionedList::GetItemIDFromRow(int row)
{
	if ( !m_SortedItems.IsValidIndex(row) )
		return -1;
...
//-----------------------------------------------------------------------------
bool OldSectionedList::GetCellBounds(int itemID, int column, int &x, int &y, int &wide, int &tall)
{
	x = y = wide = tall = 0;
	...
	// get the item
	COldItemButton *item = m_Items[itemID];
...
//-----------------------------------------------------------------------------
void OldSectionedList::InvalidateItem(int itemID)
{
	if ( !IsItemIDValid(itemID) )
		return;
...
//-----------------------------------------------------------------------------
void OldSectionedList::EnterEditMode(int itemID, int column, vgui::Panel *editPanel)
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::LeaveEditMode()
{
	if (m_hEditModePanel.Get())
	{
...
//-----------------------------------------------------------------------------
bool OldSectionedList::IsInEditMode()
{
...
//-----------------------------------------------------------------------------
void OldSectionedList::SetImageList(ImageList *imageList, bool deleteImageListWhenDone)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void OldSectionedList::OnSetFocus()
{
	if (m_hSelectedItem.Get())
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
int OldSectionedList::GetSectionTall()
{
	if (m_Sections.Count())
	{
...
//-----------------------------------------------------------------------------
void OldSectionedList::GetContentSize(int &wide, int &tall)
{
...
//-----------------------------------------------------------------------------
int OldSectionedList::GetNewItemButton()
{
	int itemID = m_Items.AddToTail();
	if (m_FreeItems.Count())
	{
		// reusing an existing COldItemButton
	...
	{
		// create a new COldItemButton
		m_Items[itemID] = SETUP_PANEL(new COldItemButton(this, itemID));
	}
	return itemID;
}
