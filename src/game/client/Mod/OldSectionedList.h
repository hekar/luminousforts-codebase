...
#ifndef OldSectionedList_H
#define OldSectionedList_H
...
class COldSectionHeader;
class COldItemButton;
class OldSectionedList;

// sorting function, should return true if itemID1 should be displayed before itemID2
typedef bool (*SectionSortFunc_t)(OldSectionedList *list, int itemID1, int itemID2);

//-----------------------------------------------------------------------------
// Purpose: List panel control that is divided up into discrete sections
//-----------------------------------------------------------------------------
class OldSectionedList : public Panel
{
	DECLARE_CLASS_SIMPLE( OldSectionedList, Panel );

public:
	OldSectionedList(vgui::Panel *parent, const char *name);
	~OldSectionedList();
...
private:
	...
	void AddSectionHelper(int sectionID, COldSectionHeader *header, SectionSortFunc_t sortFunc);
	int GetSectionTall();
	...
	friend COldItemButton;
	void SetSelectedItem(COldItemButton *item);
	DHANDLE<COldItemButton> m_hSelectedItem;
	...
	struct section_t
	{
		...
		COldSectionHeader *m_pHeader;
		CUtlVector<column_t> m_Columns;
		SectionSortFunc_t m_pSortFunc;
	};

	CUtlVector<section_t> 				m_Sections;
	CUtlLinkedList<COldItemButton *, int> 	m_Items;
	CUtlLinkedList<COldItemButton *, int> 	m_FreeItems;
    CUtlVector<COldItemButton *> 			m_SortedItems;
