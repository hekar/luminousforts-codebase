...
//	memset( m_iPacketloss, 0, sizeof( m_iPacketloss ) );
	...
	m_Colors[SDK_TEAM_BLUE] = Color( LF_CLR_TEAM_BLUE );
	m_Colors[SDK_TEAM_RED] = Color( LF_CLR_TEAM_RED );
	m_Colors[TEAM_UNASSIGNED] = Color( LF_CLR_TEAM_UNASSIGNED );
...
const Color &C_PlayerResource::GetTeamColor(int index )
{
	...
	{
		AssertMsg( false, "Invalid Color from get teamcolor" );
		static Color blah;
		return blah;
	}
	else
