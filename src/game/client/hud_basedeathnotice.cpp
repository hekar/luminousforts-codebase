...
void CHudBaseDeathNotice::Paint()
{
	...
	int ySpacer = 90; // Spacing for Timer and Phase Controller
	int yStart = pViewport->GetDeathMessageStartHeight() + ySpacer;
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		int iconWide = 0, iconTall = 0, iconActualWide = 0;

		int iconWidthLeeway = 0;
		if (icon->bRenderUsingFont)
		{
			// Give some extra room to the HL2DM icons
			iconWidthLeeway= 64;
		}
		...
		if ( icon )
		{			
			iconActualWide = icon->EffectiveWidth( 1.0f ) + iconWidthLeeway;
		...
		if ( icon )
		{
			icon->DrawSelf( x, yIcon, iconWide, iconTall, m_clrIcon );
			x += iconWide;
		}

		int iDeathInfoOffset = 0;
		int iVictimTextOffset = 5;
