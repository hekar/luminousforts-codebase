...
void CSatchelCharge::CreateEffects( void )
{
	...
	int teamnum = GetThrower()->GetTeamNumber();

	// Create a blinking light to show we're an active SLAM
	if ( teamnum == TEAM_RED )
	{
		m_hGlowSprite = CSprite::SpriteCreate( "sprites/redglow1.vmt", GetAbsOrigin(), false );
	}
	else
	{
		m_hGlowSprite = CSprite::SpriteCreate( "sprites/blueglow1.vmt", GetAbsOrigin(), false );
	}
