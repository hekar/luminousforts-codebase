	...
	float flStartTime, int nDissolveType, bool *pRagdollCreated )
{
	...
	if ( (pTarget->IsPlayer()) )
	{
		CSDKPlayer *pPlayer = dynamic_cast <CSDKPlayer *> (pTarget);

		if (!pPlayer->CanBecomeRagdoll())
			return NULL;

		CTakeDamageInfo info;
		CBaseEntity *pRagdoll = CreateServerRagdoll( pPlayer, 0, info, COLLISION_GROUP_DEBRIS, true );
...
void CEntityDissolve::DissolveThink( void )
{
	...
	if ( dt >= m_flFadeOutStart + m_flFadeOutLength )
	{
		...
		CTakeDamageInfo info( pPlayer, pPlayer, 10000.0, DMG_GENERIC | iNoPhysicsDamage );
