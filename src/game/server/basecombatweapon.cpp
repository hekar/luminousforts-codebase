...
public:
	...
	virtual bool ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask )
	{
		...
		if ( pEntity->GetCollisionGroup() == COLLISION_GROUP_WEAPON )
			return false;
		else if ( pEntity->GetCollisionGroup() == COLLISION_GROUP_FLAG )
			return false;
...
void CBaseCombatWeapon::FallInit( void )
{
	...
	else
	{
		...
		// Constrained start?
		if ( HasSpawnFlags( LF_WEAPON_START_CONSTRAINED ) )
		{
...
{ 
	...
	if ( !IsFollowingEntity() && !HasSpawnFlags(LF_WEAPON_NO_PLAYER_PICKUP) )
	{
