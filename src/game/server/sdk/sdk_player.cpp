...
static float SDKDamageForce( const Vector &size, float damage )
{ 
...
LINK_ENTITY_TO_CLASS( sdkplayer, CSDKPlayer );
PRECACHE_REGISTER( sdkplayer );

// Kohan: Adds SDKPushawayThink to the Datadesc table so it doesn't asplode with asserts.
...
BEGIN_SEND_TABLE_NOBASE( CSDKPlayerShared, DT_SDKPlayerShared )
	...
	SendPropFloat( SENDINFO( m_flStamina ), 0, SPROP_ROUNDUP | SPROP_CHANGES_OFTEN, 0.0f, 100.0f ),
#endif
	SendPropBool( SENDINFO( m_bAllowedToSprint ) ),
...
//	SendPropAngle( SENDINFO_VECTORELEM(m_angEyeAngles, 1), 10, SPROP_CHANGES_OFTEN ),
	SendPropInt( SENDINFO( m_iMaxHealth ), 12, SPROP_UNSIGNED ),
END_SEND_TABLE()
...
void CSDKPlayer::SetAbsVelocity( const Vector &vecAbsVelocity )
{
	BaseClass::SetAbsVelocity( vecAbsVelocity );
}

bool CSDKPlayer::CanHearAndReadChatFrom( CBasePlayer *pPlayer )
{
	// Override dead and nondead players talking
...
{
	// Physcannon fix
	// Special Thanks to Sub-Zero1324
...
{
	SetModel( pszPossiblePlayerModels[0] );	//Tony; basically, leave this alone ;) unless you're not using classes or teams, then you can change it to whatever.
	...
	else if (ClassicGameRules()->IsBuildPhase())
	{
		EquipSuit( false );
	}

	if ( MultiplayRules()->IsIntermission() )
	{
		AddFlag( FL_FROZEN );
	}
	else
...
int CSDKPlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	{
		Vector force = vecDir * -SDKDamageForce( WorldAlignSize(), info.GetBaseDamage() );
		if ( force.z > 500.0f )
		{
			force.z = 500.0f;
		}
...
void CSDKPlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_DISSOLVE )
	{
		...
		Assert( Dissolve( NULL, gpGlobals->curtime, false, nDissolveType ) );
	}
	else
	{
