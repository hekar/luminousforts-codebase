...
//Hekar: HACKHACK -- create one of these and fake a keyvalue to get the right explosion setup
	...
	const EHANDLE *ignoredEntity , Class_T ignoredClass )
{
	...
	if ( pOwner->GetTeamNumber() != SDK_TEAM_SPECTATOR )
	{
		Q_snprintf( buf,sizeof(buf), "%3d", magnitude );
	}
	else
	{
		Q_snprintf( buf,sizeof(buf), "0" );
	}

	pExplosion->KeyValue( szKeyName, buf );
	...
	else
	{
		if ( radius )
		{
			pExplosion->KeyValue( "iRadiusOverride", "0" );
		}

		pExplosion->KeyValue( "DamageForce", "0.0" );
	}
