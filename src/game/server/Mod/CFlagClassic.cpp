...
{
	CSDKTeam *pTeam = GetGlobalSDKTeam( ( GetTeamNumber() == SDK_TEAM_RED ) ? SDK_TEAM_BLUE : SDK_TEAM_RED );
	if ( !pTeam )
	{
...
void CFlagClassic::OnPickup()
{
	if ( !lf_reverse_flags.GetBool() && !m_pPlayerWithFlag->HasTouched() && !m_bFirstTake )
	{
		...
		GetGlobalSDKTeam( m_pPlayerWithFlag->GetTeamNumber() )->AddCapturePoints( FLAG_CLASSIC_TOUCH_POINTS );
	}
