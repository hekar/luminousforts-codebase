...
void CBlockBase::Spawn()
{
	...
	{
		CSDKTeam *pTeam = static_cast< CSDKTeam * >( GetTeam() );
...
void CBlockBase::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason != PUNTED_BY_CANNON )
	{
		if ( GetTeamNumber() != pPhysGunUser->GetTeamNumber() )
		{
			...
			CSDKTeam *pNewTeam = (CSDKTeam *) pPhysGunUser->GetTeam();
		...
		CSDKPlayer *pPlayer = dynamic_cast< CSDKPlayer * > ( pPhysGunUser );
		if ( pPlayer )
		{
...
{
	GetGlobalSDKTeam( GetTeamNumber() )->AddBlockCount( -GetBlockWorth() );
...
PrivateMethod void CBlockBase::SetLastFrozenUserID( int UserID )
{
	m_iUserIDFreezer = UserID;
}

PrivateMethod void CBlockBase::SetLastUnFrozenUserID( int UserID )
{
