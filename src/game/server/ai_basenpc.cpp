	...
	if ( modelKeyValues->LoadFromBuffer( modelinfo->GetModelName( GetModel() ), buf ) )
	{
		...
		if ( pkvInteractions )
		{
			...
			while ( pkvNode )
			{
				...
				if ( pszAngles )
				{
					sInteraction.iFlags |= SCNPC_FlagProp_TEST_OTHER_ANGLES;
				...
				if ( pszVelocity )
				{
					sInteraction.iFlags |= SCNPC_FlagProp_TEST_OTHER_VELOCITY;
				...
				{
					sInteraction.iFlags |= SCNPC_FlagProp_LOOP_IN_ACTION;
				}
				...
				if ( pszDontFixup )
				{
					...
					{
						sInteraction.iFlags |= SCNPC_FlagProp_DONT_TELEPORT_AT_END_ME;
					}
					else if ( !Q_stricmp( pszDontFixup, "them" ) || !Q_stricmp( pszDontFixup, "both" ) ) 
					{
						sInteraction.iFlags |= SCNPC_FlagProp_DONT_TELEPORT_AT_END_THEM;
					}
				}
				...
				if ( pszNeedsWeapon )
				{
					...
					{
						sInteraction.iFlags |= SCNPC_FlagProp_NEEDS_WEAPON_ME;
					}
					else if ( !Q_strncmp( pszNeedsWeapon, "THEM", 4 ) ) 
					{
						sInteraction.iFlags |= SCNPC_FlagProp_NEEDS_WEAPON_THEM;
					}
...
void CAI_BaseNPC::StartScriptedNPCInteraction( CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction, Vector vecOtherOrigin, QAngle angOtherAngles )
{
	...
	if ((pInteraction->iFlags & SCNPC_FlagProp_DONT_TELEPORT_AT_END_ME) != 0)
	{
	 	pMySequence->AddSpawnFlags( SF_SCRIPT_DONT_TELEPORT_AT_END );
	}
	pMySequence->SetLoopActionSequence( (pInteraction->iFlags & SCNPC_FlagProp_LOOP_IN_ACTION) != 0 );
	...
	if ( pOtherNPC )
	{
		...
		if ((pInteraction->iFlags & SCNPC_FlagProp_DONT_TELEPORT_AT_END_THEM) != 0) 
		{
			pTheirSequence->AddSpawnFlags( SF_SCRIPT_DONT_TELEPORT_AT_END );
		}
		pTheirSequence->SetLoopActionSequence( (pInteraction->iFlags & SCNPC_FlagProp_LOOP_IN_ACTION) != 0 );
...
void CAI_BaseNPC::CalculateValidEnemyInteractions( void )
{
	...
	for ( int i = 0; i < m_ScriptedInteractions.Count(); i++ )
	{
		...
		if ( pInteraction->iFlags & SCNPC_FlagProp_NEEDS_WEAPON_ME )
		{
			if ( !GetActiveWeapon())
				continue;
		...
		if ( pInteraction->iFlags & SCNPC_FlagProp_NEEDS_WEAPON_THEM )
		{
...
bool CAI_BaseNPC::InteractionCouldStart( CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction, Vector &vecOrigin, QAngle &angAngles )
{
	...
	if ( pInteraction->iFlags & SCNPC_FlagProp_TEST_OTHER_ANGLES )
	{
	...
	if ( pInteraction->iFlags & SCNPC_FlagProp_TEST_OTHER_VELOCITY )
	{
